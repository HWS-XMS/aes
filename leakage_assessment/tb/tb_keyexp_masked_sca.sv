...
// ============================================================================
// tb_keyexp_masked_sca - leakage (SCA) testbench for the DOM-masked AES-128 KEY
// SCHEDULE (KEY_EXPANSION_128_MASKED), N shares (compile-time +define+NSHARES=<n>).
//
...
//     LAT active cycles (SubWord / SBOX_MASKED cascade; fresh randomness)
...
module tb_keyexp_masked_sca;
    ...
    KEY_EXPANSION_128_MASKED #(N) dut (
    ...
    initial begin
        ...
        sca_init("config_keyexp_masked.yaml");
        sca_set_metadata("dut", $sformatf("keyexp_masked_n%0d", N));
        ...
        sca_save(outfile);
        $display("[tb_keyexp_masked_sca] N=%0d group=%s traces=%0d cycles=%0d -> %s",
