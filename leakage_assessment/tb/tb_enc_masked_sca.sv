...
// ============================================================================
// tb_enc_masked_sca - leakage (SCA) testbench for the DOM-masked AES-128 encrypt
// DATAPATH (AES_ENCRYPT_MASKED), N shares (compile-time +define+NSHARES=<n>).
//
...
module tb_enc_masked_sca;
    ...
    initial begin
        ...
        sca_init("config_enc_masked.yaml");
        sca_set_metadata("dut", $sformatf("enc_masked_n%0d", N));
        ...
        sca_save(outfile);
        $display("[tb_enc_masked_sca] N=%0d group=%s traces=%0d cycles=%0d -> %s",
