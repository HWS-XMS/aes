...
// ROUND_MASKED_TB - masked AES round vs unmasked ROUND (registered), N = 2, 3.
// recombine(masked) == unmasked for random state/key, fresh randomness.  Latency = 8.
module ROUND_MASKED_TB;
    ...
    initial begin
        ...
        for (t = 0; t < 200; t++) begin
            ...
            `CHK_EQ("round N2", rec2(so2), ref_out);
            `CHK_EQ("round N3", rec3(so3), ref_out);
        end
        `TB_SUMMARY("ROUND_MASKED_TB");
    end
endmodule
