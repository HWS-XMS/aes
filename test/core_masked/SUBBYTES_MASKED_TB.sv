import AES_PKG::*;
import AES_MASKED_PKG::*;
// SUBBYTES_MASKED_TB - masked SubBytes vs unmasked SUBBYTES, N = 2, 3.  recombine(masked)
// == unmasked for random states with fresh randomness each cycle.  Latency = 8.
module SUBBYTES_MASKED_TB;
    `include "tb_check.svh"

    logic clk = 0;
    logic rst = 1;
    always #5 clk = ~clk;

    function automatic logic [127:0] r128;
        r128 = {$random, $random, $random, $random};
    endfunction

    logic [127:0] st;
    logic [127:0] ref_out;
    SUBBYTES uref (
        .state_in  (st     ),
        .state_out (ref_out)
    );

    localparam int N2  = 2;
    localparam int N3  = 3;
    localparam int RT2 = 16*sbox_rand_words(N2)*8;
    localparam int RT3 = 16*sbox_rand_words(N3)*8;
    logic [N2*128-1:0] ss2;
    logic [N2*128-1:0] so2;
    logic [RT2-1:0]    rn2;
    logic [N3*128-1:0] ss3;
    logic [N3*128-1:0] so3;
    logic [RT3-1:0]    rn3;
    SUBBYTES_MASKED #(N2) s2 (
        .clk       (clk),
        .rst       (rst),
        .state_in  (ss2),
        .rnd       (rn2),
        .state_out (so2)
    );
    SUBBYTES_MASKED #(N3) s3 (
        .clk       (clk),
        .rst       (rst),
        .state_in  (ss3),
        .rnd       (rn3),
        .state_out (so3)
    );

    integer kk;
    always @(posedge clk) begin
        for (kk = 0; kk < RT2/32; kk++) begin
            rn2[kk*32 +: 32] <= $random;
        end
        for (kk = 0; kk < RT3/32; kk++) begin
            rn3[kk*32 +: 32] <= $random;
        end
    end

    function automatic logic [127:0] rec2(input logic [N2*128-1:0] v);
        rec2 = v[0+:128] ^ v[128+:128];
    endfunction
    function automatic logic [127:0] rec3(input logic [N3*128-1:0] v);
        rec3 = v[0+:128] ^ v[128+:128] ^ v[256+:128];
    endfunction

    integer t;
    logic [127:0] m0;
    logic [127:0] m1;
    initial begin
        repeat (3) @(posedge clk); #1;
        rst = 0;
        for (t = 0; t < 200; t++) begin
            st = r128();
            m0 = r128();
            ss2[0+:128]   = st ^ m0;
            ss2[128+:128] = m0;
            m0 = r128();
            m1 = r128();
            ss3[0+:128]   = st ^ m0 ^ m1;
            ss3[128+:128] = m0;
            ss3[256+:128] = m1;
            repeat (12) @(posedge clk); #1;   // 8-cycle latency + margin; st held
            `CHK_EQ("sb N2", rec2(so2), ref_out);
            `CHK_EQ("sb N3", rec3(so3), ref_out);
        end
        `TB_SUMMARY("SUBBYTES_MASKED_TB");
    end
endmodule
