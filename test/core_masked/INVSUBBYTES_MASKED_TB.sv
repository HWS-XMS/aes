...
// INVSUBBYTES_MASKED_TB - masked InvSubBytes vs unmasked INVSUBBYTES, N = 2, 3.
// recombine(masked) == unmasked for random states, fresh randomness.  Latency = 8.
module INVSUBBYTES_MASKED_TB;
    ...
    initial begin
        ...
        for (t = 0; t < 200; t++) begin
            ...
            ss3[128+:128] = m0;
            ss3[256+:128] = m1;
            repeat (12) @(posedge clk); #1;   // 8-cycle latency + margin; st held
            `CHK_EQ("isb N2", rec2(so2), ref_out);
            `CHK_EQ("isb N3", rec3(so3), ref_out);
        end
        `TB_SUMMARY("INVSUBBYTES_MASKED_TB");
    end
endmodule
