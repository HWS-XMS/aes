...
// LASTROUND_MASKED_TB - masked final round vs unmasked LASTROUND (registered), N = 2, 3.
// recombine(masked) == unmasked for random state/key, fresh randomness.  Latency = 8.
module LASTROUND_MASKED_TB;
    ...
    initial begin
        ...
        for (t = 0; t < 200; t++) begin
            ...
            `CHK_EQ("last N2", rec2(so2), ref_out);
            `CHK_EQ("last N3", rec3(so3), ref_out);
        end
        `TB_SUMMARY("LASTROUND_MASKED_TB");
    end
endmodule
