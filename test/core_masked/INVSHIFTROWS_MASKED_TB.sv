...
// INVSHIFTROWS_MASKED_TB - masked InvShiftRows, N = 2, 3, 4.  recombine(masked) ==
// unmasked INVSHIFTROWS for random state with random share splits.  Combinational.
module INVSHIFTROWS_MASKED_TB;
    ...
    INVSHIFTROWS_MASKED #(N2) s2 (
        .state_in  (ss2),
        .state_out (so2)
    );
    INVSHIFTROWS_MASKED #(N3) s3 (
        .state_in  (ss3),
        .state_out (so3)
    );
    INVSHIFTROWS_MASKED #(N4) s4 (
        .state_in  (ss4),
        .state_out (so4)
    );
    ...
    initial begin
        for (t = 0; t < 500; t++) begin
            ...
            ss4[384+:128] = m2;
            #1;
            `CHK_EQ("isr N2", rec2(so2), ref_out);
            `CHK_EQ("isr N3", rec3(so3), ref_out);
            `CHK_EQ("isr N4", rec4(so4), ref_out);
        end
        `TB_SUMMARY("INVSHIFTROWS_MASKED_TB");
    end
endmodule
