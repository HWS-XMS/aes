...
// ADDROUNDKEY_MASKED_TB - masked AddRoundKey, N = 2, 3, 4.  recombine(masked) ==
// unmasked ADDROUNDKEY for random state/key with random share splits.  Combinational.
module ADDROUNDKEY_MASKED_TB;
    ...
    ADDROUNDKEY_MASKED #(N2) a2 (
        .state_in  (ss2),
        .key_in    (ks2),
        .state_out (so2)
    );
    ADDROUNDKEY_MASKED #(N3) a3 (
        .state_in  (ss3),
        .key_in    (ks3),
        .state_out (so3)
    );
    ADDROUNDKEY_MASKED #(N4) a4 (
        .state_in  (ss4),
        .key_in    (ks4),
        .state_out (so4)
    );
    ...
    initial begin
        ...
        `TB_SUMMARY("ADDROUNDKEY_MASKED_TB");
    end
endmodule
