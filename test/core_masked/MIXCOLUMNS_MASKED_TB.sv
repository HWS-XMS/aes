...
// MIXCOLUMNS_MASKED_TB - masked MixColumns, N = 2, 3, 4.  recombine(masked) == unmasked
// MIXCOLUMNS for random state with random share splits.  Combinational.
module MIXCOLUMNS_MASKED_TB;
    ...
    MIXCOLUMNS_MASKED #(N2) s2 (
        .state_in  (ss2),
        .state_out (so2)
    );
    MIXCOLUMNS_MASKED #(N3) s3 (
        .state_in  (ss3),
        .state_out (so3)
    );
    MIXCOLUMNS_MASKED #(N4) s4 (
        .state_in  (ss4),
        .state_out (so4)
    );
    ...
    initial begin
        for (t = 0; t < 500; t++) begin
            ...
            ss4[384+:128] = m2;
            #1;
            `CHK_EQ("mc N2", rec2(so2), ref_out);
            `CHK_EQ("mc N3", rec3(so3), ref_out);
            `CHK_EQ("mc N4", rec4(so4), ref_out);
        end
        `TB_SUMMARY("MIXCOLUMNS_MASKED_TB");
    end
endmodule
