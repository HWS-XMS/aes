...
// INVROUND_MASKED_TB - masked inverse round vs unmasked INVROUND (registered), N = 2, 3.
// recombine(masked) == unmasked for random state/key, fresh randomness.  Latency = 8.
module INVROUND_MASKED_TB;
    ...
    initial begin
        ...
        for (t = 0; t < 200; t++) begin
            ...
            `CHK_EQ("invround N2", rec2(so2), ref_out);
            `CHK_EQ("invround N3", rec3(so3), ref_out);
        end
        `TB_SUMMARY("INVROUND_MASKED_TB");
    end
endmodule
