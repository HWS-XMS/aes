...
// AES_ENCRYPT_MASKED_TB - tapped DOM-masked encrypt datapath, N = 2, all three sizes.
// Round-key shares from KEY_EXPANSION_MASKED (key_rnd = 0 -> stable) muxed by size;
// datapath runs rnd = 0.  Recombined ciphertext vs OpenSSL (taps 9/11/13).
module AES_ENCRYPT_MASKED_TB;
    ...
    initial begin
        ...
        `CHK("done", 1);
        `TB_SUMMARY("AES_ENCRYPT_MASKED_TB");
    end
endmodule
