import AES_PKG::*;
// INVMIXCOLUMN_TB - single-column InvMixColumn on FIPS-197 round-1 column 0
// (reversed) + MixColumn(InvMixColumn(c)) == c over random columns.
module INVMIXCOLUMN_TB;
    `include "tb_check.svh"

    column_t ci, co, rt;

    INVMIXCOLUMN dut (.col_in(ci), .col_out(co));
    MIXCOLUMN    fwd (.col_in(co), .col_out(rt));

    integer t;
    initial begin
        ci[0] = 8'h04; ci[1] = 8'h66; ci[2] = 8'h81; ci[3] = 8'he5; #1;
