...
// KEY_EXPANSION_256_MASKED_TB - DOM-masked AES-256 key schedule, N = 2.  Key split into
// 2 random shares; recombined round-key shares vs NIST reference (keyexp256_w.mem).
module KEY_EXPANSION_256_MASKED_TB;
    ...
    KEY_EXPANSION_256_MASKED #(N2) e2 (
        .clk        (clk ),
        .rst        (rst ),
        .key_valid  (kvld),
        .key        (k2  ),
    ...
    initial begin
        ...
        `CHK("read keys", i > 0);
        `TB_SUMMARY("KEY_EXPANSION_256_MASKED_TB");
    end
endmodule
