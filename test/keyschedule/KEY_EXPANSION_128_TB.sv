import AES_PKG::*;
// KEY_EXPANSION_128_TB - full AES-128 key schedule vs the NIST reference
// (test/ref/keyexp128_*.mem: keys + 44 expanded words each, validated against
// FIPS-197 Appendix A).  round_keys is flattened so bytes can be addressed by a
// computed bit position: round_keys[r][c][b] = rk_flat[r*128 + c*32 + b*8 +: 8].
module KEY_EXPANSION_128_TB;
    ...
    KEY_EXPANSION_128 dut (.key(key), .round_keys(round_keys));
    ...
    initial begin
        ...
        `CHK("read keys", i > 0);
        `TB_SUMMARY("KEY_EXPANSION_128_TB");
    end
endmodule
