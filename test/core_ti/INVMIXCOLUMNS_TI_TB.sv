import AES_PKG::*;
import AES_TI_PKG::*;
// INVMIXCOLUMNS_TI_TB - masked InvMixColumns, N = 2, 3, 4.  recombine(masked) ==
// unmasked INVMIXCOLUMNS for random state with random share splits.  Combinational.
module INVMIXCOLUMNS_TI_TB;
    ...
    INVMIXCOLUMNS_TI #(N2) s2 (
        .state_in  (ss2),
        .state_out (so2)
    );
    INVMIXCOLUMNS_TI #(N3) s3 (
        .state_in  (ss3),
        .state_out (so3)
    );
    INVMIXCOLUMNS_TI #(N4) s4 (
        .state_in  (ss4),
        .state_out (so4)
    );
    ...
    initial begin
        ...
        `TB_SUMMARY("INVMIXCOLUMNS_TI_TB");
    end
endmodule
