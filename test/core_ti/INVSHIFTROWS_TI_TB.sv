import AES_PKG::*;
import AES_TI_PKG::*;
// INVSHIFTROWS_TI_TB - masked InvShiftRows, N = 2, 3, 4.  recombine(masked) ==
// unmasked INVSHIFTROWS for random state with random share splits.  Combinational.
module INVSHIFTROWS_TI_TB;
    ...
    INVSHIFTROWS_TI #(N2) s2 (
        .state_in  (ss2),
        .state_out (so2)
    );
    INVSHIFTROWS_TI #(N3) s3 (
        .state_in  (ss3),
        .state_out (so3)
    );
    INVSHIFTROWS_TI #(N4) s4 (
        .state_in  (ss4),
        .state_out (so4)
    );
