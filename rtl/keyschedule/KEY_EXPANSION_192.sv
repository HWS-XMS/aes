...
module KEY_EXPANSION_192 #(
    ...
    // Expanded key columns (52 columns for AES-192: (Nr+1)*Nb = 13*4 = 52)
    column_t [((Nr+1)*Nb)-1:0] key_columns;
    
    // Initial key columns - direct mapping from input key
    // Key bytes are in order: k0,k1,k2,...,k23
    ...
    generate
        for (k = 0; k < Nk; k++) begin : gen_initial_key
            assign key_columns[k][0] = key[23 - (k*4 + 0)];
            assign key_columns[k][1] = key[23 - (k*4 + 1)];
            assign key_columns[k][2] = key[23 - (k*4 + 2)];
            assign key_columns[k][3] = key[23 - (k*4 + 3)];
        end
    endgenerate
