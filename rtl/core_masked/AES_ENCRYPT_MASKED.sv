...
// AES_ENCRYPT_MASKED - fully-unrolled DOM-masked encrypt datapath, runtime keysize.
// Thirteen ROUND_MASKED stages (each 8 cycles); the size taps the share-state after
// 9/11/13 rounds into one LASTROUND_MASKED.  Round keys are the N-share store; rnd is
// fresh DOM randomness every cycle.  Latency = Nr*SBOX_LAT.
module AES_ENCRYPT_MASKED #(
    parameter  int N     = 2,
