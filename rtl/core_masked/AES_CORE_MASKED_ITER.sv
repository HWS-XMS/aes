...
// AES_CORE_MASKED_ITER - iterative (rolled) DOM-masked keyed AES core: one masked round
// datapath reused per encrypt/decrypt for minimal area / clean SCA measurement.  Interface
// is 1:1 with the unrolled AES_CORE_MASKED (drop-in).  One FSM (IDLE/KEYEXP/ENC/DEC; ready,
// key_loaded; priority update_key > encrypt > decrypt).  Round-key shares stored from
// KEY_EXPANSION_MASKED; the iterative datapaths take one round's rnd, driven fresh/cycle.
module AES_CORE_MASKED_ITER #(
    parameter  int N     = 2,
